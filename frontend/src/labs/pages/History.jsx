...
const HistoryDetailModal = ({ data, onClose }) => {
    ...
    const isGlaucoma =
        resultStr.includes("GLAUCOMA") || resultStr.includes("GLAUKOMA");
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            doc.text("MEDICAL EXAMINATION REPORT", 105, 20, {
                align: "center",
            });
            ...
            doc.text("GlaucoScan AI Diagnostic System v1.0", 105, 25, {
                align: "center",
            });
            ...
            const baseUrl =
                "https://mollusklike-intactly-kennedi.ngrok-free.dev/api/proxy-image?path=fundus_images/";
            const fundusUrl = data.image_url
                ? `${baseUrl}${data.image_url.split("/").pop()}`
                : null;
            const heatmapUrl = data.heatmap_url
                ? `${baseUrl}${data.heatmap_url.split("/").pop()}`
                : null;
            ...
            if (fundusUrl) {
                try {
                    ...
                    doc.text("[Fundus image load failed]", 20, imageY + 25);
                }
            }
            ...
            if (heatmapUrl) {
                try {
                    ...
                    doc.setFontSize(8);
                    doc.text(
                        "Fig 2: AI Localization (Heatmap)",
                        110,
                        imageY + 60,
                    );
                } catch (e) {
                    doc.text("[Heatmap load failed]", 110, imageY + 25);
                }
            }
            ...
            const advice =
                data.advice ||
                "No specific clinical notes provided. Regular follow-up is recommended.";
            ...
            doc.text(
                "This is a computer-generated report by GlaucoScan AI.",
                20,
                280,
            );
                ...
                <div className="p-8 overflow-y-auto flex-1 bg-gray-50/50">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-2 space-y-8">
                            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                                ...
                                <div className="flex gap-6 items-center mb-8">
                                    <div
                                        className={`p-6 rounded-2xl text-center min-w-[160px] ${isGlaucoma ? "bg-red-50 text-red-700 border border-red-100" : "bg-green-50 text-green-700 border border-green-100"}`}
                                    >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full transition-all duration-1000 ${isGlaucoma ? "bg-red-500" : "bg-green-500"}`}
                                                style={{
                                                    width: `${displayScore}%`,
                                                }}
                                            ></div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2 text-center">
                                        <img
                                            src={
                                                data.image_url
                                                    ? `https://mollusklike-intactly-kennedi.ngrok-free.dev/storage/fundus_images/${data.image_url.split("/").pop()}`
                                    ...
                                    <div className="space-y-2 text-center">
                                        <img
                                            src={
                                                data.heatmap_url
                                                    ? `https://mollusklike-intactly-kennedi.ngrok-free.dev/storage/fundus_images/${data.heatmap_url.split("/").pop()}`
                                                    : "https://picsum.photos/400"
                                            }
                                            className="w-full aspect-video object-cover rounded-2xl border"
                                            alt="Heatmap"
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                                ...
                                <p className="p-5 bg-blue-50/50 border border-blue-100 rounded-2xl text-blue-900 italic text-sm leading-relaxed">
                                    "
                                    {data.advice ||
                                        "Analisis AI menunjukkan kondisi retina saat ini."}
                                    "
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <button
                                onClick={generatePDF}
                                className="w-full py-4 bg-green-600 text-white rounded-2xl font-bold shadow-lg flex items-center justify-center gap-2 hover:bg-green-700 transition-all"
                            >
        ...
        <div className="p-6">
            <div className="flex justify-between items-end mb-8">
                <div>
                    ...
                    <p className="text-gray-500">Arsip data medis AI</p>
                </div>
                <button
                    onClick={exportExcel}
                    className="bg-green-600 text-white px-5 py-2.5 rounded-xl font-semibold flex items-center gap-2 hover:bg-green-700 transition-all shadow-lg"
                >
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 border-b border-gray-50 flex flex-wrap gap-4 items-center">
                    <div className="relative flex-1 min-w-[300px]">
                        ...
                        />
                        <input
                            className="w-full pl-12 pr-4 py-2.5 rounded-xl border outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div className="overflow-x-auto">
                    {loading ? (
                        <div className="p-20 text-center text-gray-400 animate-pulse italic font-bold">
                            Sinkronisasi Data...
                        </div>
                    ) : (
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {historyList.map((item, i) => {
                                    const res =
                                        item.result?.toUpperCase() || "NORMAL";
                                    const isG = res.includes("GLAU");
                                    const score = isG
                                        ? Number(item.conf)
                                        : 100 - Number(item.conf);
                                    return (
                                        <tr
                                            key={item.id}
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4">
                                                <span
                                                    className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest ${isG ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                                                >
                                                    {res}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                                        <div
                                                            className={`h-full ${isG ? "bg-red-500" : "bg-green-500"}`}
                                                            style={{
                                                                width: `${score}%`,
                                                            }}
                                                        ></div>
