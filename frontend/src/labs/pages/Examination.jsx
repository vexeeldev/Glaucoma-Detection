...
const BASE_STORAGE_URL =
    "https://mollusklike-intactly-kennedi.ngrok-free.dev/storage/";
...
const Examination = () => {
        ...
        const fetchPatientDetail = async () => {
            ...
            try {
                ...
                const response = await axios.get(
                    `labs/examination-detail/${appointmentId}`,
                    {
                        headers: {
    ...
    const handleUpload = async () => {
        if (!selectedFile)
            return alert("Silakan pilih gambar retina terlebih dahulu.");
    ...
    const handleFinish = async () => {
        ...
        try {
            ...
            finalData.append(
                "confidence_score",
                (apiData.confidence_score * 100).toFixed(1),
            );
            ...
            finalData.append(
                "medical_advice",
                apiData.medical_advice || "Tetap jaga kondisi kesehatan mata.",
            );
            finalData.append("image", selectedFile);

            const response = await axios.post(
                `labs/examination/${appointmentId}/result`,
                finalData,
                {
                    headers: {
            ...
            if (response.data.status === "success") {
                alert(
                    "Seluruh rekam medis dan data antrean berhasil diperbarui!",
                );
    ...
    if (loadingPatient)
        return (
            <div className="p-20 text-center font-bold text-blue-600 animate-pulse">
                Menghubungkan ke Ruang Periksa...
            </div>
        );
    ...
    return (
        <div className="max-w-4xl mx-auto animate-in slide-in-from-bottom-4 duration-500 pb-20">
            ...
            <div className="mb-8 flex items-center gap-4">
                <button
                    onClick={() => navigate("/labs/dashboard")}
            ...
            <div className="flex items-center justify-between mb-12 relative px-10">
                ...
                {[1, 2, 3].map((s) => (
                    <div
                        key={s}
                        className="flex flex-col items-center gap-3 bg-[#f8fafc] px-2"
                    >
                        <div
                        ...
                        <div>
                            ...
                            <div className="mt-6 flex items-start gap-3 text-sm text-gray-500 p-4 border border-dashed border-gray-200 rounded-2xl">
                                ...
                                <span className="font-medium">
                                    Catatan: {patient?.medical_history}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                    <div className="mb-10 text-center">
                        <label className="block text-sm font-bold text-gray-700 mb-4 uppercase tracking-widest">
                            Upload Foto Retina: {patient?.name}
                        </label>
                        <div className="border-2 border-dashed border-gray-200 rounded-3xl p-12 text-center hover:border-blue-400 hover:bg-blue-50/50 transition-all cursor-pointer group relative overflow-hidden">
                            ...
                            {previewUrl ? (
                                <div className="relative">
                                    <img
                                        src={previewUrl}
                                        className="h-64 rounded-2xl shadow-md mx-auto object-cover border-4 border-white"
                                        alt="Preview"
                                    />
                                    <p className="mt-4 text-blue-600 font-bold bg-blue-50 inline-block px-4 py-1 rounded-full">
                                        {selectedFile?.name}
                                    </p>
                                </div>
                            ) : (
                                <>
                                    ...
                                    <p className="text-sm text-gray-400">
                                        Dukung file JPG, PNG, atau TIFF (Maks.
                                        10MB)
                                    </p>
                                </>
                            )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10 text-left">
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    {["Kiri", "Kanan", "Keduanya"].map(
                                        (opt) => (
                                            <button
                                                type="button"
                                                key={opt}
                                                onClick={() =>
                                                    setSelectedEyeSide(opt)
                                                }
                            ...
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-3 uppercase tracking-tighter text-[10px]">
                                    Model Alat Kamera
                                </label>
                                <input
                                    type="text"
                                    className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 font-medium text-sm"
                        ...
                        <div>
                            ...
                            <textarea
                                className="w-full px-4 py-3 rounded-xl border border-gray-200 h-[126px] bg-gray-50"
                                placeholder="Misal: Cahaya redup karena pupil tidak melebar..."
                            ></textarea>
                        </div>
                    </div>
                    ...
                    {mlLoading ? (
                        <div className="bg-white p-24 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
                            ...
                            <h2 className="text-2xl font-bold text-gray-900 mb-2">
                                AI sedang menganalisis retina...
                            </h2>
                            <p className="text-gray-500 max-w-sm font-medium">
                                Sistem sedang menghitung probabilitas glaukoma
                                untuk {patient?.name}.
                            </p>
                        </div>
                    ) : (
                        <div className="space-y-6">
                            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                                <div className="flex flex-col md:flex-row gap-8 items-center mb-10">
                                    ...
                                    >
                                        <p className="text-xs font-black uppercase tracking-[0.2em] text-gray-400 mb-3 text-center">
                                            Hasil Sistem AI
                                        </p>
                                        <h2
                                            className={`text-5xl font-black mb-1 tracking-tighter ${!isGlaucoma ? "text-green-700" : "text-red-700"}`}
                                        >
                                            {resultLabel}
                                        </h2>
                                        <div
                                            className={`flex items-center gap-2 font-bold uppercase text-xs mt-2 ${!isGlaucoma ? "text-green-600" : "text-red-600"}`}
                                        >
                                            {!isGlaucoma ? (
                                                <CheckCircle size={18} />
                                            ) : (
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div className="space-y-3">
                                                <div className="aspect-square bg-gray-50 rounded-3xl overflow-hidden border-4 border-white shadow-md">
                                                    <img
                                                        src={
                                                            apiData?.file_path
                                                                ? `${BASE_STORAGE_URL}${apiData.file_path}`
                                                                : previewUrl
                                                        }
                                            ...
                                            <div className="space-y-3">
                                                <div className="aspect-square bg-gray-50 rounded-3xl overflow-hidden border-4 border-white shadow-md relative">
                                                    ...
                                                    />
                                                    <div
                                                        className={`absolute inset-0 ${isGlaucoma ? "bg-red-500/30" : "bg-blue-500/20"} mix-blend-overlay`}
                                                    ></div>
                                                </div>
                                                <p className="text-[9px] font-black text-gray-400 uppercase">
                                                    Heatmap
                            ...
                            <div className="flex gap-4">
                                <button
                                    type="button"
                                    className="flex-1 py-4 bg-white border-2 border-gray-200 rounded-2xl font-black text-[10px] uppercase tracking-widest text-gray-500 hover:bg-gray-50 transition-all active:scale-95 shadow-sm flex items-center justify-center gap-2"
                                >
